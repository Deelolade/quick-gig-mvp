        ...
        <>
            <div className='flex justify-between   '>
                ...
                <div className="dashboard lg:w-[85%] w-full bg-gray-100 h-[auto] md:pb-20 ">
                    <nav className='h-[10vh] lg:w-[85vw] w-full py-4 px-5 md:px-12 flex justify-between items-center bg-white shadow-md fixed z-20' >
                        ...
                        <div className='max-w-4xl h-[84vh] w-full overflow-y-auto overflow-x-hidden flex flex-col px-2 py-4 space-y-4'>
                            ...
                            {messages.map((message, idx) => (
                                <div key={idx} className={`flex ${message.senderId?._id === currentUser._id ? 'justify-end' : 'items-start'} my-2`}>
                                    {/* Check if sender exists before trying to render their profile picture */}
                                    {message.senderId?._id && (
                                        <img
                                            src={message.senderId?.profilePicture}
                                            alt="Sender"
                                            className={`  ${message.senderId?._id === currentUser._id ? 'hidden ' : 'block'}w-12 h-12 rounded-full me-3`}
                                        />
                                    )}
                                    ...
                                    >
                                        <div className="">
                                            {/* If sender exists, show username, else show "Deleted User" */}
                                            <p className='font-bold'>
                                                {message.senderId?._id === currentUser._id ? "You" : message.senderId?.userName || "Deleted User"}
                                            </p>
                                            <p>{message.text}</p>
                                    ...
                                    {/* Check if sender exists before rendering their profile picture */}
                                    {message.senderId?._id === currentUser._id && (
                                        <img
                                            src={message.senderId?.profilePicture}
                                            alt="Sender"
                                            className={`hidden ms-2 w-12 h-12 rounded-full me-3`}
                                        />
                                    )}
